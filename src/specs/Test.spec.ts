...
test.describe('Calendly', () => {
  test('book the next available timeslot', async ({ page }) => {
    ...
    await ai('cancel the privacy model on the screen', { page, test })
...
test.describe('Wikipedia', () => {
  test('view article history and verify earliest revision', async ({ page }) => {
    ...
    const date = await ai('find the first revision listed on this page?', { page, test })

    expect(date).toEqual('1. 20:57, 1 January 2001')
  })
})
